...
namespace PacmanEditor
{
    public unsafe class PointeredImage
    {
        ...
        private nint ptr;
            ...
            public void DrawBitmapTile(int x, int y, int w, int h, int xSource, int ySource, int palSource, PointeredImage source, byte mx, byte my, byte pallenght = 16)
            {
                for (int xl = 0; xl < w; xl++)
                {
                    for (int yl = 0; yl < h; yl++)
                    {
                        ...
                        //{
                            ...
                            this[xx + x, yy + y] = (byte)(source[xSource + xl, ySource + yl] + (palSource * pallenght));
                        //}
                    }
                }
            }
            ...
            public void DrawBitmapTile(int x, int y, PointeredImage source, ushort tile,byte pal = 0, bool invis = false)
            {
                ...
                for (int xl = 0; xl < 8; xl++)
                {
                    for (int yl = 0; yl < 8; yl++)
                    {
                        ...
                        this[xx + x, yy + y] = (byte)((source[xSource + xl, ySource + yl])+pal*16);
